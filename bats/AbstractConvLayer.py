...
class AbstractConvLayer(AbstractLayer):
    def __init__(self, neurons_shape: np.ndarray, name: str = "", use_padding: bool = False, padding: list[int] = [0,0], is_residual: bool = False):
        n_neurons = neurons_shape[0] * neurons_shape[1] * neurons_shape[2]
        super(AbstractConvLayer, self).__init__(n_neurons, name, is_residual=is_residual)
